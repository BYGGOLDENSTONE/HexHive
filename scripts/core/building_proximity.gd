...
extends Node
## Detects all buildings within 2 hex tiles of the hero.
## On each hero hex change, clears and re-emits all nearby buildings.

@onready var hex_grid: HexGrid = %HexGrid
@onready var hero: Hero = %Hero

## Detection range in hex distance.
const RANGE: int = 1

## Previously tracked nearby building coords.
...
func _check_proximity(hero_hex: Vector2i) -> void:
	...
	# Emit new nearby buildings
	for coord in current_coords:
		var b: Variant = hex_grid.get_building_at(coord)
		if b != null:
			SignalBus.hero_near_building.emit(b as Node2D, coord)
